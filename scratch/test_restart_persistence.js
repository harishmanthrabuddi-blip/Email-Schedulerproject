...
async function runRestartPersistenceTests() {
  ...
  try {
    ...
    const [rows1] = await pool.query('SELECT status, queue_job_id FROM emails WHERE id = ?', [email1Id]);
    ...
    // Simulate backend restart by querying via recovery logic
    const recoveryModule = require('../backend/dist/services/emailRecoveryService');
    await recoveryModule.reconcileScheduledEmails();

    const job1After = await queue.getJob(job1Id);
    assert(job1After !== null, 'BullMQ job #1 still exists after recovery reconciliation');

    console.log('  Waiting 18 seconds for worker processing...');
    await new Promise((res) => setTimeout(res, 18000));

    const [afterRows1] = await pool.query('SELECT status, sent_at FROM emails WHERE id = ?', [email1Id]);
    ...
    // Note: We intentionally do NOT add job to Redis (or remove it if present) to simulate missing Redis job
    ...
    console.log('  Waiting 18 seconds for recovered job execution...');
    await new Promise((res) => setTimeout(res, 18000));

    const [afterRows2] = await pool.query('SELECT status, sent_at FROM emails WHERE id = ?', [email2Id]);
    ...
    console.log('  Waiting 5 seconds for immediate worker processing...');
    await new Promise((res) => setTimeout(res, 5000));

    const [afterRows3] = await pool.query('SELECT status FROM emails WHERE id = ?', [email3Id]);
    ...
    console.log('  Waiting 28 seconds to verify email #4 is sent exactly once...');
    await new Promise((res) => setTimeout(res, 28000));

    const [afterRows4] = await pool.query('SELECT status, attempts FROM emails WHERE id = ?', [email4Id]);
    ...
    console.log('  Waiting 16 seconds for worker to process 5A & 5B with minimum delay spacing...');
    await new Promise((res) => setTimeout(res, 16000));

    const [rows5A] = await pool.query('SELECT status, sent_at FROM emails WHERE id = ?', [insert5A.insertId]);
    const [rows5B] = await pool.query('SELECT status, sent_at FROM emails WHERE id = ?', [insert5B.insertId]);
